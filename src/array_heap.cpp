...
array_heap::array_heap(std::uint32_t size)
: heap_size(size)
{
	_nodes =std::vector<heap_node>(heap_size, heap_node{0,0});
    mutual_idxes = std::vector<std::uint32_t>(heap_size, 0);
	reset();
    // todo sort heap
}
bool array_heap::empty() const
{
    return remain_heap_num == 0;
}

void array_heap::maintain_heap(std::uint32_t idx)
{
...
void array_heap::swap_node(std::uint32_t index_one, std::uint32_t index_two)
{
...
void array_heap::update_by_dec(std::uint32_t pos)
{
...
void array_heap::update_by_inc(std::uint32_t pos)
{
	auto k = mutual_idxes[pos];
	auto& cur_node = _nodes[k];
	cur_node.value++;
...
std::ostream& operator<<(std::ostream& output, const array_heap& data)
{
